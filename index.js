...
async function generateSession(res, method, phoneNumber = null) {
    ...
    try {
        ...
        timeout = setTimeout(() => {
            if (!res.headersSent) res.status(500).send(getPage('<h1>Error</h1><p>Connection timed out. The server is likely blocked.</p>'));
            cleanup();
        }, 60000);
        ...
        bot.ev.on('connection.update', async (update) => {
            ...
            if (connection === 'open') {
                ...
                if (method === 'pair') {
                    ...
                    if (!res.headersSent) res.send(getPage(`<h1>Pairing Code</h1><p>Your code is:</p><h2 class="message">${code}</h2><p>Enter this in WhatsApp. Session will be sent after you connect.</p>`));
                }

                bot.ev.once('connection.update', async (finalUpdate) => {
                    if (finalUpdate.connection === 'open') {
                        if (!res.headersSent) res.write(getPage(`<h1>Success!</h1><p>Paired successfully. Sending session to your WhatsApp...</p>`));
                        const zip = new JSZip();
                        ...
                        if (!res.headersSent) res.end(getPage(`<h1>Done!</h1><p>Session ID has been sent to your WhatsApp.</p>`));
                        cleanup();
